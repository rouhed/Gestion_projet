...
export default function TaskDetailModal({ taskId, projectMembers, onClose, onTaskUpdated, currentUser }) {
  ...
  const handleDeleteTask = () => {
    setCustomConfirm({
      ...
      message: 'Êtes-vous sûr de vouloir supprimer cette tâche ? Cette action est irréversible et supprimera également tous ses commentaires.',
      onConfirm: async () => {
        try {
  ...
  const canChangeStatus = isMyTask || isUserAdmin;
  const canEditMetadata = isUserAdmin;
        ...
        <div className="modal-body" style={{ display: 'grid', gridTemplateColumns: '1.7fr 1fr', gap: '2.5rem', paddingBottom: '1rem' }}>
          ...
          <div>
            ...
            <div style={{ marginBottom: '2rem', background: 'rgba(0, 0, 0, 0.15)', padding: '1rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-glass)' }}>
              ...
              {/* Formulaire ajout d'heures et minuteur */}
              {isMyTask && (
              ...
              <div className="comments-list">
                {comments.length === 0 ? (
                  ...
                  comments.map(comment => {
                    const canDelete = isUserAdmin || (currentUser && comment.author?.id === currentUser.id);
                    return (
        ...
        <div className="modal-overlay" style={{ zIndex: 11000 }} onClick={() => setCustomConfirm({ ...customConfirm, isOpen: false })}>
          <div className="modal-content" style={{ maxWidth: '420px', textAlign: 'center', padding: '2rem' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem' }}>
              <button className="btn btn-secondary" onClick={() => setCustomConfirm({ isOpen: false, title: '', message: '', onConfirm: null })}>Annuler</button>
              <button 
                className="btn btn-danger" 
                onClick={() => {
      ...
      {customAlert.isOpen && (
        <div className="modal-overlay" style={{ zIndex: 11000 }} onClick={() => setCustomAlert({ ...customAlert, isOpen: false })}>
          <div className="modal-content" style={{ maxWidth: '420px', textAlign: 'center', padding: '2rem' }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: '1.5' }}>{customAlert.message}</p>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button className="btn btn-primary" onClick={() => setCustomAlert({ isOpen: false, title: 'Erreur', message: '' })}>D'accord</button>
            </div>
          </div>
        </div>
      )}
